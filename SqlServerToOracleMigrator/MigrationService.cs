                        ...
                        if (additionalColumns != null && additionalColumns.Count > 0)
                        {
                            ...
                            for (int ai = 0; ai < additionalColumns.Count; ai++)
                            {
                                ...
                                if (ai < (additionalColumnsValues?.Count ?? 0))
                                {
                                    ...
                                    // {Col} 토큰이 있으면 파라미터로 변환
                                    if (builtExpr.Contains(":p"))
                                    {
                                        foreach (System.Text.RegularExpressions.Match match in System.Text.RegularExpressions.Regex.Matches(rawExpr, @"\{([^}]+)\}"))
                                        {
                                            ...
                                            if (dataTable.Columns.Contains(colName))
                                            {
                                                ...
                                                {
                                                    parameters.Add(CreateOracleParameter($":p{paramIndex}", DBNull.Value));
                                                }
                                                else
                                                {
                                                    parameters.Add(CreateOracleParameter($":p{paramIndex}", colValue));
                                                }
                                                paramIndex++;
                                            }
                                        }
                                    ...
                                    if (additionalColumns != null)
                                    {
                                        ...
                                        foreach (var addCol in additionalColumns)
                                        {
                                            ...
                                            var builtExpr = (additionalColumnsValues != null && ai < additionalColumnsValues.Count)
                                                ? BuildAdditionalExpressionForInsert(additionalColumnsValues[ai], mappedColumns.Cast<object>().ToList(), validColumns, row)
                                                : "NULL";
